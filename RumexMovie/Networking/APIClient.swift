...
class APIClient {
    ...
    static func getMovies(page: Int, completion:@escaping (Result<GetMovieResponce, AFError>)->Void) {
        performRequest(route: APIRouter.popular(page: page), completion: completion)
    }
    
    static func getTVShows(page: Int, completion:@escaping (Result<GetMovieResponce, AFError>)->Void) {
        performRequest(route: APIRouter.upcoming(page: page), completion: completion)
    }
    
    static func getSearchMovies(query: String, completion:@escaping (Result<GetMovieResponce, AFError>)->Void) {
        performRequest(route: APIRouter.search(query: query), completion: completion)
    }
